...
function Page({ id }) {
  ...
  if (error) {
    return (
      <Layout title='Artist Detail - MyBook'>
  ...
  return (
    <Layout
      title={`${data ? data[0]?.name + ' - MyBook' : 'Artist Detail - MyBook'}`}
      description={`${data ? 'Browse song and album by ' + data[0]?.name + ' - MyBook' : 'Artist Detail - MyBook'}`}
    >
      <div className='mb-10'>
        {data ? (
          <>
